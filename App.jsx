...
import {
  ...
  Printer, ChevronRight, Navigation, Upload, FolderOpen,
  Download, Eye, Activity, Target, Zap,
} from "lucide-react";
  ...
  return(
    <div style={{flex:1,overflowY:"auto",padding:"28px 32px",background:"#f1f4fb"}}>
      ...
      <div className="au2" style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:12,marginBottom:20}}>
        ...
        <KpiCard icon={Clock} color={AMBER} label="Por cobrar" value={fmtK(pendiente)} sub={facts.filter(f=>f.status==="Pendiente").length+" facturas pendientes"} onClick={()=>setView("facturas")}/>
        <KpiCard icon={Map} color={VIOLET} label="Rutas activas" value={rutas.filter(r=>r.status==="En curso").length} sub={rutas.length+" rutas total"} onClick={()=>setView("rutas")}/>
      </div>
