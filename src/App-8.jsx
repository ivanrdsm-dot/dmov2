        ...
        <div style={{display:"flex",flexDirection:"column",gap:14}}>
          ...
          {modo==="foraneo"&&<>
            <div style={{background:"#ffffff",border:"1px solid "+BORDER,borderRadius:15,padding:20,boxShadow:"0 1px 4px rgba(12,24,41,.04)"}}>
              ...
              {fCiudades.map((c,i)=>(
                <div key={c.id} style={{display:"flex",alignItems:"center",gap:9,padding:"10px 13px",background:A+"06",border:"1.5px solid "+A+"22",borderRadius:10,marginBottom:8}}>
                  ...
                  <div style={{flex:1}}>
                    ...
                    <div style={{fontFamily:MONO,fontSize:10,color:MUTED}}>{c.km.toLocaleString()} km · {fmt(c[fVeh]||0)}</div>
                  </div>
        ...
        <div style={{position:"sticky",top:78,display:"flex",flexDirection:"column",gap:13}}>
          <div style={{background:"#ffffff",border:"1.5px solid "+BORDER,borderRadius:16,overflow:"hidden",boxShadow:"0 4px 20px rgba(12,24,41,.07)"}}>
            ...
            <div style={{padding:"14px 18px"}}>
              {modo==="foraneo"&&<>
                {fCiudades.map(c=><Row key={c.id} l={"📍 "+c.c} v={fmt(c[fVeh]||0)}/>)}
    ...
    <div style={{flex:1,overflowY:"auto",padding:"28px 32px",background:"#f1f4fb"}}>
      ...
      {modal&&<Modal title="Nuevo registro de facturación" onClose={()=>setModal(false)} icon={FileText} iconColor={BLUE} wide>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
          ...
          <div>
            <div style={{fontSize:10,fontWeight:700,color:MUTED,marginBottom:5,textTransform:"uppercase",letterSpacing:"0.05em"}}>Plan al que se carga</div>
            <select value={form.plan} onChange={sf("plan")} style={{width:"100%",background:"#fff",border:"1.5px solid "+BORDER2,borderRadius:9,padding:"9px 12px",fontSize:13}}>
