...
function Cotizador({onSaved}){
  ...
  const canQ=modo==="foraneo"?fCiudades.length>0:modo==="masivo"?mPDV>0:true;
...
function PlanificadorNacional(){
  ...
  const [ciudades,setCiudades]=useState([]); // {id,ciudad,estado,pdv,dias,vans,capDia}
  const [search,setSearch]=useState("");
  const [fileRows,setFileRows]=useState(null); // parsed excel rows
  ...
  const calcCiudad=(pdv,dias,mpd)=>{
    const capDia=Math.min(mpd,Math.ceil(pdv/Math.max(dias,1)));
    const vans=Math.ceil(pdv/(capDia*Math.max(dias,1)));
    return{vans:Math.max(1,vans),capDia};
  };
  ...
  const updCiudad=(id,k,v)=>{
    setCiudades(p=>p.map(c=>{
      if(c.id!==id) return c;
      const upd={...c,[k]:v};
  ...
  // Recalc all when maxPDia changes
  useEffect(()=>{
  ...
  const parseFile=async(file)=>{
    ...
    if(name.endsWith(".csv")){
      ...
      if(rows.length<2){showT("CSV vacío","err");return;}
      setFileRows(rows);
      processRows(rows);
    } else {
      ...
      // Simple XLSX text extraction (basic)
      showT("Sube el archivo como CSV (.csv) para mejor compatibilidad","warn");
    }
  };
  ...
  const processRows=(rows)=>{
    ...
    const ciIdx=header.findIndex(h=>h.includes("ciudad")||h.includes("city"));
    const stIdx=header.findIndex(h=>h.includes("estado")||h.includes("state"));
    const pdvIdx=header.findIndex(h=>h.includes("pdv")||h.includes("punto")||h.includes("entrega")||h.includes("qty")||h.includes("cantidad"));
    const diasIdx=header.findIndex(h=>h.includes("dia")||h.includes("day"));

    if(ciIdx===-1&&pdvIdx===-1){showT("No se encontraron columnas Ciudad/PDV en el archivo","err");return;}

    const nuevas=rows.slice(1).map(r=>{
      const ciudad=ciIdx>=0?r[ciIdx]?.trim()||"":"";
      const estado=stIdx>=0?r[stIdx]?.trim()||"":"";
      const pdv=Math.abs(parseInt(r[pdvIdx>=0?pdvIdx:1])||0);
      const dias=diasIdx>=0?parseInt(r[diasIdx])||diasTotal:diasTotal;
      if(!ciudad&&pdv===0) return null;
      const {vans,capDia}=calcCiudad(pdv,dias,maxPDia);
      return{id:uid(),ciudad,estado,pdv,dias,vans,capDia};
    }).filter(Boolean);

    // Group by city (same city → sum PDVs)
    const grouped={};
    nuevas.forEach(c=>{
      const key=(c.ciudad+"|"+c.estado).toLowerCase();
      if(grouped[key]) grouped[key].pdv+=c.pdv;
      else grouped[key]={...c};
    });
    const arr=Object.values(grouped).map(c=>{const{vans,capDia}=calcCiudad(c.pdv,c.dias,maxPDia);return{...c,vans,capDia};});
    setCiudades(arr);
    showT("✓ "+arr.length+" ciudades importadas del archivo");
  };
  ...
  // Sort ciudades by state for display
  const ciudadesSorted=[...ciudades].sort((a,b)=>(a.estado+a.ciudad).localeCompare(b.estado+b.ciudad));

  // Group by estado
  const porEstado={};
  ciudadesSorted.forEach(c=>{const e=c.estado||"Sin estado";if(!porEstado[e]) porEstado[e]=[];porEstado[e].push(c);});
    ...
    <div style={{flex:1,overflowY:"auto",padding:"28px 32px",background:"#f1f4fb"}}>
      ...
      <div className="au" style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:22}}>
        <div>
          ...
          <p style={{color:MUTED,fontSize:13,marginTop:3}}>Distribución simultánea · Multi-ciudad · Optimizador de flota</p>
        </div>
        <div style={{display:"flex",gap:8}}>
          {tab==="nuevo"&&ciudades.length>0&&<button onClick={saveProyecto} className="btn" style={{display:"flex",alignItems:"center",gap:7,background:"linear-gradient(135deg,"+GREEN+",#059669)",color:"#fff",borderRadius:11,padding:"9px 16px",fontWeight:700,fontSize:13}}>
          ...
          <button onClick={()=>setTab(tab==="nuevo"?"lista":"nuevo")} className="btn" style={{display:"flex",alignItems:"center",gap:7,background:tab==="lista"?A+"10":"#fff",border:"1.5px solid "+(tab==="lista"?A:BORDER2),borderRadius:11,padding:"9px 16px",fontWeight:700,fontSize:13,color:tab==="lista"?A:TEXT}}>
            {tab==="nuevo"?<><FolderOpen size={13}/>Mis proyectos ({proyectos.length})</>:<><Plus size={13}/>Nuevo proyecto</>}
          </button>
        </div>
      </div>

      {tab==="lista"&&<>
        {proyectos.length===0?<div style={{background:"#fff",border:"1px solid "+BORDER,borderRadius:15,padding:60,textAlign:"center",color:MUTED}}>
          ...
          {proyectos.map(p=>(
            <div key={p.id} style={{background:"#fff",border:"1px solid "+BORDER,borderRadius:14,overflow:"hidden"}}>
              <div style={{padding:"14px 16px",borderBottom:"1px solid "+BORDER,background:"linear-gradient(135deg,"+VIOLET+"08,"+BLUE+"05)"}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
                  <div>
                    <div style={{fontFamily:DISPLAY,fontWeight:700,fontSize:15,color:TEXT}}>{p.nombre}</div>
                    {p.cliente&&<div style={{fontSize:12,color:MUTED,marginTop:2}}>{p.cliente}</div>}
                  </div>
                  <button onClick={()=>delProyecto(p.id)} className="btn" style={{color:MUTED,padding:4}}><Trash2 size={13}/></button>
                </div>
              </div>
              <div style={{padding:"12px 16px",display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8}}>
                {[[A,"PDVs totales",p.totPDV?.toLocaleString()||"0"],[VIOLET,"Vans necesarias",p.totVans||"0"],[BLUE,"Ciudades",p.ciudades?.length||0]].map(([c,l,v])=>(
              ...
              {p.ciudades&&p.ciudades.length>0&&<div style={{padding:"0 16px 14px"}}>
                <div style={{fontSize:10,color:MUTED,fontWeight:700,textTransform:"uppercase",marginBottom:6}}>Ciudades</div>
                <div style={{display:"flex",flexWrap:"wrap",gap:4}}>
                  {p.ciudades.slice(0,6).map(c=><span key={c.id} style={{background:VIOLET+"10",color:VIOLET,borderRadius:6,padding:"2px 7px",fontSize:10,fontWeight:600}}>{c.ciudad}</span>)}
                  {p.ciudades.length>6&&<span style={{fontSize:10,color:MUTED}}>+{p.ciudades.length-6} más</span>}
                </div>
              </div>}
            </div>
          ))}
      ...
      {tab==="nuevo"&&<>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr 1fr",gap:12,marginBottom:16}}>
          <div style={{background:"#fff",border:"1px solid "+BORDER,borderRadius:13,padding:"14px 16px",gridColumn:"1/-1",display:"grid",gridTemplateColumns:"2fr 1fr 1fr 1fr",gap:12}}>
            ...
            <div>
              ...
              <input type="number" min="1" max="200" value={maxPDia} onChange={e=>setMaxPDia(Math.max(1,parseInt(e.target.value)||1))}
            ...
            <div>
              ...
              <input type="number" min="1" max="90" value={diasTotal} onChange={e=>setDiasTotal(Math.max(1,parseInt(e.target.value)||1))}
          ...
          <div style={{display:"flex",flexDirection:"column",gap:14}}>
            ...
            <div style={{background:"#fff",border:"1px solid "+BORDER,borderRadius:14,overflow:"hidden"}}>
              <div style={{padding:"14px 18px",borderBottom:"1px solid "+BORDER,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                <span style={{fontFamily:DISPLAY,fontWeight:700,fontSize:14}}>📂 Importar desde archivo</span>
                <Tag color={BLUE}>CSV · Excel</Tag>
              </div>
              <div style={{padding:16}}>
                <div
                  ...
                  style={{border:"2px dashed "+(dragOver?A:BORDER2),borderRadius:12,padding:"28px 20px",textAlign:"center",cursor:"pointer",background:dragOver?A+"05":"#fafbfd",transition:"all .15s"}}>
                  <Upload size={28} color={dragOver?A:MUTED} style={{margin:"0 auto 10px"}}/>
                  <div style={{fontWeight:700,fontSize:14,color:dragOver?A:TEXT}}>Arrastra tu archivo aquí</div>
                  <div style={{fontSize:12,color:MUTED,marginTop:4}}>o haz clic para seleccionar · CSV o Excel (.csv, .xlsx)</div>
                  <div style={{marginTop:10,fontSize:11,color:MUTED,fontStyle:"italic"}}>Columnas esperadas: Ciudad, Estado, PDVs (o Cantidad), Días</div>
                </div>
                ...
                {ciudades.length>0&&<div style={{marginTop:10,padding:"8px 13px",background:GREEN+"08",borderRadius:9,border:"1px solid "+GREEN+"20",fontSize:12,color:GREEN,fontWeight:600}}>
                  ✓ {ciudades.length} ciudades cargadas · {totPDV.toLocaleString()} PDVs totales
                </div>}
                <div style={{marginTop:12,padding:"10px 13px",background:"#f8fafd",borderRadius:9,border:"1px solid "+BORDER}}>
                  <div style={{fontSize:10,fontWeight:800,color:MUTED,textTransform:"uppercase",marginBottom:6}}>Formato CSV esperado:</div>
                  <code style={{fontSize:11,color:TEXT,fontFamily:MONO}}>Ciudad,Estado,PDVs,Dias<br/>Monterrey,Nuevo León,384,3<br/>Guadalajara,Jalisco,251,2</code>
                </div>
              </div>
            </div>

            {/* Agregar ciudad manual */}
            <div style={{background:"#fff",border:"1px solid "+BORDER,borderRadius:14,overflow:"hidden"}}>
              <div style={{padding:"14px 18px",borderBottom:"1px solid "+BORDER}}>
                <span style={{fontFamily:DISPLAY,fontWeight:700,fontSize:14}}>➕ Agregar ciudad manualmente</span>
              </div>
              <div style={{padding:14,display:"grid",gridTemplateColumns:"2fr 1fr 1fr 1fr auto",gap:9,alignItems:"end"}}>
                <div>
                  <div style={{fontSize:9,fontWeight:700,color:MUTED,marginBottom:4,textTransform:"uppercase"}}>Ciudad *</div>
                  <input value={addC} onChange={e=>setAddC(e.target.value)} placeholder="Ej: Monterrey"
                    style={{width:"100%",background:"#fff",border:"1.5px solid "+BORDER2,borderRadius:8,padding:"8px 11px",fontSize:13}}/>
                </div>
                <div>
                  <div style={{fontSize:9,fontWeight:700,color:MUTED,marginBottom:4,textTransform:"uppercase"}}>Estado</div>
                  <input value={addE} onChange={e=>setAddE(e.target.value)} placeholder="Nuevo León"
                    style={{width:"100%",background:"#fff",border:"1.5px solid "+BORDER2,borderRadius:8,padding:"8px 11px",fontSize:13}}/>
                </div>
                <div>
                  <div style={{fontSize:9,fontWeight:700,color:MUTED,marginBottom:4,textTransform:"uppercase"}}>PDVs *</div>
                  <input type="number" value={addP} onChange={e=>setAddP(e.target.value)} placeholder="384"
                    style={{width:"100%",background:"#fff",border:"1.5px solid "+BORDER2,borderRadius:8,padding:"8px 11px",fontFamily:MONO,fontSize:13,fontWeight:700,color:A}}/>
                </div>
                <div>
                  <div style={{fontSize:9,fontWeight:700,color:MUTED,marginBottom:4,textTransform:"uppercase"}}>Días</div>
                  <input type="number" value={addD} onChange={e=>setAddD(e.target.value)} placeholder={diasTotal}
                    style={{width:"100%",background:"#fff",border:"1.5px solid "+BORDER2,borderRadius:8,padding:"8px 11px",fontFamily:MONO,fontSize:13,fontWeight:700,color:BLUE}}/>
                </div>
                <button onClick={addCiudad} className="btn" style={{background:A,color:"#fff",borderRadius:9,padding:"8px 14px",fontWeight:700,fontSize:13,cursor:"pointer",whiteSpace:"nowrap"}}>
                  <Plus size={13}/>
                </button>
              </div>
            </div>
            ...
            {ciudades.length>0&&<div style={{background:"#fff",border:"1px solid "+BORDER,borderRadius:14,overflow:"hidden"}}>
              <div style={{padding:"13px 18px",borderBottom:"1px solid "+BORDER,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                ...
                <div style={{display:"flex",gap:7}}>
                  <Tag color={A}>{totPDV.toLocaleString()} PDVs</Tag>
                  <Tag color={VIOLET}>{totVans} vans</Tag>
                  <Tag color={BLUE}>{Object.keys(porEstado).length} estados</Tag>
                </div>
              </div>
              <div style={{overflowX:"auto"}}>
                <table style={{width:"100%",borderCollapse:"collapse",minWidth:600}}>
                  <thead><tr style={{background:"#f8fafd",borderBottom:"1px solid "+BORDER}}>
                    {["Ciudad","Estado","PDVs","Días","Vans","PDVs/día","Acciones"].map(h=>(
                      <th key={h} style={{padding:"8px 13px",textAlign:"left",fontSize:9,color:MUTED,fontWeight:800,letterSpacing:"0.06em",textTransform:"uppercase"}}>{h}</th>
                    ))}
                  </tr></thead>
                  <tbody>
                    {Object.entries(porEstado).map(([estado,cits])=>[
                      <tr key={"h_"+estado}><td colSpan={7} style={{padding:"7px 13px",background:VIOLET+"06",fontSize:10,fontWeight:800,color:VIOLET,letterSpacing:"0.06em",textTransform:"uppercase",borderBottom:"1px solid "+BORDER}}>{estado} · {cits.length} ciudades · {cits.reduce((a,c)=>a+c.pdv,0).toLocaleString()} PDVs · {cits.reduce((a,c)=>a+c.vans,0)} vans</td></tr>,
                      ...cits.map(c=>(
                        <tr key={c.id} className="fr" style={{borderBottom:"1px solid "+BORDER}}>
                          <td style={{padding:"9px 13px",fontWeight:700,fontSize:13}}>{c.ciudad}</td>
                          <td style={{padding:"9px 13px",fontSize:12,color:MUTED}}>{c.estado||"—"}</td>
                          <td style={{padding:"9px 13px"}}>
                            <input type="number" value={c.pdv} onChange={e=>updCiudad(c.id,"pdv",Math.max(0,parseInt(e.target.value)||0))}
                              style={{width:75,background:"#fff",border:"1.5px solid "+A+"28",borderRadius:7,padding:"4px 8px",fontFamily:MONO,fontSize:13,fontWeight:700,color:A}}/>
                          </td>
                          <td style={{padding:"9px 13px"}}>
                            <input type="number" value={c.dias} onChange={e=>updCiudad(c.id,"dias",Math.max(1,parseInt(e.target.value)||1))}
                              style={{width:60,background:"#fff",border:"1.5px solid "+BLUE+"28",borderRadius:7,padding:"4px 8px",fontFamily:MONO,fontSize:13,fontWeight:700,color:BLUE}}/>
                          </td>
                          <td style={{padding:"9px 13px",fontFamily:MONO,fontSize:14,fontWeight:800,color:VIOLET}}>{c.vans}</td>
                          <td style={{padding:"9px 13px",fontFamily:MONO,fontSize:12,color:MUTED}}>{c.capDia}/día</td>
                          <td style={{padding:"9px 13px"}}>
                            <button onClick={()=>rmCiudad(c.id)} className="btn" style={{color:MUTED,padding:"3px 5px"}}><X size={12}/></button>
                          </td>
                        </tr>
                      ))
                    ])}
          ...
          <div style={{position:"sticky",top:20,display:"flex",flexDirection:"column",gap:12}}>
            <div style={{background:"#fff",border:"1px solid "+BORDER,borderRadius:14,overflow:"hidden"}}>
              <div style={{borderTop:"3px solid "+VIOLET,padding:"16px 18px 12px"}}>
                <div style={{fontFamily:MONO,fontSize:9,color:VIOLET,letterSpacing:"0.12em",textTransform:"uppercase",marginBottom:8}}>● RESUMEN DEL PROYECTO</div>
                <div style={{fontFamily:MONO,fontWeight:800,fontSize:36,color:TEXT,lineHeight:1}}>{totPDV.toLocaleString()}</div>
                <div style={{fontSize:11,color:MUTED,marginTop:4}}>PDVs totales en {ciudades.length} ciudades</div>
              </div>
              <div style={{padding:"12px 18px",borderTop:"1px solid "+BORDER}}>
                <Row l="🏙️ Ciudades" v={ciudades.length}/>
                <Row l="🗺️ Estados" v={Object.keys(porEstado).length}/>
                <Row l="🚛 Vans simultáneas" v={totVans} c={VIOLET}/>
                <Row l="📦 Max PDVs/van/día" v={maxPDia}/>
                <Row l="⏱️ Días máx operación" v={totDias} c={BLUE}/>
              </div>
              {ciudades.length>0&&<div style={{padding:"0 18px 16px"}}>
                ...
                {Object.entries(porEstado).map(([e,cits])=>(
                  <div key={e} style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
                    <div>
                      <div style={{fontSize:12,fontWeight:700,color:TEXT}}>{e}</div>
                      <div style={{fontSize:10,color:MUTED}}>{cits.length} ciudades · {cits.reduce((a,c)=>a+c.vans,0)} vans</div>
                    </div>
                    <div style={{textAlign:"right"}}>
                      <div style={{fontFamily:MONO,fontSize:13,fontWeight:700,color:A}}>{cits.reduce((a,c)=>a+c.pdv,0).toLocaleString()}</div>
                      <div style={{fontSize:9,color:MUTED}}>PDVs</div>
                    </div>
                  </div>
                ))}
              </div>}
            ...
            {ciudades.length>0&&nombre&&<button onClick={saveProyecto} className="btn" style={{background:"linear-gradient(135deg,"+GREEN+",#059669)",color:"#fff",borderRadius:12,padding:"13px 0",fontFamily:DISPLAY,fontWeight:700,fontSize:15,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:8,boxShadow:"0 4px 16px "+GREEN+"30"}}>
